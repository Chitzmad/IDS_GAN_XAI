...
# Load your dataset
input_file = 'final_processed_CICI_updated.csv'  # Replace with your dataset file name
output_file = 'data_extracted_dataset.csv'  # Name of the output file

# Read the dataset
df = pd.read_csv(input_file)

# Define the list of columns to keep
columns_to_keep = ['IPv', 'Std', 'Protocol Type', 'Magnitue', 'UDP', 'cwr_flag_number', 'HTTP', 'Tot sum', 'ack_count', 'DNS', 'Max', 'IAT', 'SSH', 'ack_flag_number', 'DHCP', 'flow_duration', 'label']  # Replace with your column list
...
for filename in os.listdir(input_dir):
    if filename.endswith('_selected_data.csv'):  # Process only files with the specific suffix
        ...
        median_value=140000
        generate_data_with_gans(sampled_data, output_dir, base_name, latent_dim, median_value)
